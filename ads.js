    ...
    fetch = async function(url, options) {
        if (typeof url === 'string') {
            if (url.includes('video-weaver')) {
                ...
                var channelName = (new URL(url))
                    .pathname.match(/([^\/]+)(?=\.\w+$)/)[0];
                UsherParams = (new URL(url))
                    .search;
                ...
                return new Promise(function(resolve, reject) {
                    var processAfter = async function(response) {
                        if (response.status == 200) {
                            ...
                            var lines = encodingsM3u8.replace('\r', '')
                                .split('\n');
...
function getStreamUrlForResolution(encodingsM3u8, resolutionInfo, qualityOverrideStr) {
    ...
    var encodingsLines = encodingsM3u8.replace('\r', '')
        .split('\n');
    var firstUrl = null;
    var lastUrl = null;
    ...
    for (var i = 0; i < encodingsLines.length; i++) {
        if (!encodingsLines[i].startsWith('#') && encodingsLines[i].includes('.m3u8')) {
            if (i > 0 && encodingsLines[i - 1].startsWith('#EXT-X-STREAM-INF')) {
                ...
                if (resolution) {
                    if (qualityOverride) {
                        var quality = resolution.toLowerCase()
                            .split('x')[1];
    ...
    if (haveAdTags) {
        ...
        if (playerType === 'proxy') {
            try {
                ...
                switch (proxyType) {
                    case 'TAB':
                        encodingsM3u8Response = await fetchWithTimeout("https://pxy2.twitchadblock.com/channel/"+CurrentChannelName,{timeout: 9000});
                        break;
                    case 'TTV LOL':
                        encodingsM3u8Response = await fetchWithTimeout('https://api.ttv.lol/playlist/' + CurrentChannelName + '.m3u8%3Fallow_source%3Dtrue' /* + encodeURIComponent(match[2])*/ , {
                            headers: {
